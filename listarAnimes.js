...
async function listarNotices(){
    let req = await fetch("listarAnimesDestaque.php")
    ...
    for(let c in res){
        ...
        div_card_high.addEventListener("click", function(){
            ...
            window.location.href = `pages/NoticiaCompleta.html?id=${noticeId}`
        })
...
async function ultimasNoticias(){
    let req = await fetch("listarAnimesUltimasNoticias.php")
    let res = await req.json()
    console.log(res)
    for(let c in res){
